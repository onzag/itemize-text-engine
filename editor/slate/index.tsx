...
import { IText, STANDARD_TEXT_NODE } from "../../serializer/types/text";
import { ICustom } from "../../serializer/types/custom";
import { IInline } from "../../serializer/types/inline";
import { CurrentElementProvider, CurrentElementRetriever } from "./current-element";
import { IVoidBlock } from "../../serializer/types/void-block";
import { IList } from "../../serializer/types/list";
import { STANDARD_PARAGRAPH } from "../../serializer/types/paragraph";
import { ITemplateArgContextDefinition, ITemplateArgHTMLDefinition, ITemplateArgTextDefinition, ITemplateArgUIHandlerDefinition } from "../../serializer/template-args";
import { ILink } from "../../serializer/types/link";
import { ITable, ITbody, ITd, ITfoot, ITr } from "../../serializer/types/table";
import { CONTAINER_CLASS_PREFIX, CUSTOM_CLASS_PREFIX, IFeatureSupportOptions, RICH_TEXT_CLASS_PREFIX, TABLE_CLASS_PREFIX } from "../../sanitizer";
 ...
 */
  ...
  metadata: string,
  /**
   * A source, either a File or Blob
   */
  src?: File | Blob;
};
 ...
 */
export interface ISlateInsertedFileInformationType {
  /**
   * The file itself
   */
  result: ISlateFile;
...
interface ISlateEditorProps {
  ...
  /**
   * The supported types that are allowed for the image
   * input
   */
  supportedImageTypes: string;

  /**
   * Used by itemize internally, for setting the last text change
   * value count for the number of characters in an efficient way
   * since the editor can count better
   * 
   * this will set a global value with the count of the last edited
   * rich text, if a validation occurs immediately after this value
   * can be retrieved right away from the global
   * 
   * while this is a hack, it's a hack done for efficiency, since consistency
   * now is expensive
   */
  cacheStoreCountGlobal?: string;
   ...
   */
  onRetrieveFile: (fileId: string) => ISlateFile;
   ...
   * @returns 
   */
  onRetrieveImage: (fileId: string) => ({ file: ISlateFile, srcset: string })
  /**
